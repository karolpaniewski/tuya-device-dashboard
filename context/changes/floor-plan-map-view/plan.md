...
- [ ] 4.5 Dragged device position persists across reload
- [ ] 4.6 Placed device node color matches its room's badge
- [ ] 4.7 Click opens modal; setpoint change reflected on Dashboard
- [ ] 4.8 Re-drag updates position; × button returns device to roster
- [ ] 4.9 Switching active site scopes devices and floor plan correctly
- [ ] 4.10 Broken image shows error state; Dashboard list/control unaffected
- [ ] 4.11 375px viewport on `/` and `/setup` shows no regression
- [ ] 4.12 Bulk operations on list/table view still work
