...
import {
	DP_CODE_MAP,
	VALVE_STATE_DP_CODE_MAP,
} from "~/server/lib/tuya/dp-codes";
...
export async function sendValveStateCommand(
	deviceId: string,
	isOpen: boolean,
): Promise<void> {
	const [device] = await db
	...
	if (
		device.productKey === null ||
		!(device.productKey in VALVE_STATE_DP_CODE_MAP)
	) {
		throw new Error("UNSUPPORTED_DEVICE");
	}
	// biome-ignore lint/style/noNonNullAssertion: productKey presence in VALVE_STATE_DP_CODE_MAP validated by guard above
	const dps = VALVE_STATE_DP_CODE_MAP[device.productKey]!;
